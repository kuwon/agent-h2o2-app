...
async def header():
    st.markdown("<h1 class='heading'>한투 퇴직 마스터</h1>", unsafe_allow_html=True)
    st.markdown(
...
async def body() -> None:
    ...
    if (
        ...
        logger.info("---*--- Creating Pension Agent ---*---")
        pension = get_pension(user_id=user_id, model_id=model_id)
        st.session_state[agent_name]["agent"] = pension
        st.session_state["selected_model"] = model_id
    else:
        pension = st.session_state[agent_name]["agent"]

    ####################################################################
    # Load Agent Session from the database
    ####################################################################
    try:
        st.session_state[agent_name]["session_id"] = pension.load_session()
    except Exception:
    ...
    ####################################################################
    if pension.memory:
        agent_runs = pension.memory.runs
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(":thinking_face: Thinking..."):
                ...
                try:
                    ...
                    run_response = await pension.arun(user_message, stream=True)
                    ...
                    if pension.run_response is not None:
                        await add_message(agent_name, "assistant", response, pension.run_response.tools)
                    else:
    ...
    # Knowledge widget
    ####################################################################
    await knowledge_widget(agent_name, pension)

    ####################################################################
    # Session selector
    ####################################################################
    await session_selector(agent_name, pension, get_pension, user_id, model_id)

    ####################################################################
    # About section
    ####################################################################
    await utilities_widget(agent_name, pension)
