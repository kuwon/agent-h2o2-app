...
def get_pension_policy(
    ...
    return Agent(
        ...
        description=dedent("""\
            정책/근거 법령/FAQ 항목을 가진 knowledge base에 근간하여 질의에 응답하고, 고객 컨텍스트에 맞춰 규정 적용(자격판정, 금액 계산)까지 수행한다.
        """),
        instructions=dedent("""
            먼저 knowledge base를 검색하여 해당 항목이 무엇인지를 찾아서 **근거(조항/일자/출처)** 정보를 확보하라.  
            그리고 거기에 붙어있는 여러가지 단서나 조건들을 이해하여 context(고객/계좌/시뮬레이션)를 적용하여 **고객 맞춤 판단 및 답변**을 제공하라.
